...
void x264_mb_encode_i4x4( x264_t *h, int idx, int i_qp )
{
    ...
    h->dctf.sub4x4_dct( dct4x4, p_src, p_dst ); // MARK DCT变换

    nz = x264_quant_4x4( h, dct4x4, i_qp, DCT_LUMA_4x4, 1, idx ); // MARK 量化
    h->mb.cache.non_zero_count[x264_scan8[idx]] = nz;
    if( nz )
    {
...
void x264_macroblock_encode( x264_t *h )
{
    ...
    else if( h->mb.i_type == I_4x4 )
    {
        ...
        for( i = h->mb.i_skip_intra ? 15 : 0 ; i < 16; i++ ) // 如果已经编码过前15个宏块, 则只需要编码最后一个
        {
