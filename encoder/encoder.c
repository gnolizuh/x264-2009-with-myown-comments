...
static int x264_encoder_encapsulate_nals( x264_t *h )
{
    ...
    nal_buffer = (uint8_t *)h->nal_buffer;
...
static int x264_slice_write( x264_t *h )
{
    ...
    if( h->sh.i_last_mb == h->mb.i_mb_count-1 )
    {
        ...
        x264_fdec_filter_row( h, h->sps->i_mb_height ); // 最后一行的宏块也要进行去区块滤波器
    }
...
static void *x264_slices_write( x264_t *h )
{
    ...
    while( h->sh.i_first_mb < h->mb.i_mb_count ) // h->mb.i_mb_count == ( width * height ) / ( 16 * 16 ) in pixels
    {
        ...
        else if( h->param.i_slice_count ) // MARK 一个frame最多拥有i_slice_count个slices
        {
            x264_emms();
            i_slice_num++;
                             ...
                             x264_picture_t *pic_out )
{
    ...
    if( h->param.i_threads > 1) // 多线程数>1时, 可同时编码多帧
    {
...
//      fprintf(stderr, "current: %p  prev: %p  oldest: %p \n", thread_current, thread_prev, thread_oldest);
    ...
    *pp_nal = NULL;
    ...
    if( h->param.i_threads > 1 )
    {
		...
		// 1> 根据参数将frame分割成slice.
		// 2> 对每个slice执行x264_slice_write().
