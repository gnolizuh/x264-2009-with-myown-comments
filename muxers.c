...
{
    yuv_input_t *h = (yuv_input_t *)malloc( sizeof(yuv_input_t) );
    if( !h )
        return -1;
...
{
    yuv_input_t *h = (yuv_input_t *)handle;
    int i_frame_total = 0;
...
int open_file_y4m( char *psz_filename, hnd_t *p_handle, x264_param_t *p_param )
{
    ...
    y4m_input_t *h = (y4m_input_t *)malloc( sizeof(y4m_input_t) );
    if( !h )
        return -1;
...
int open_file_avis( char *psz_filename, hnd_t *p_handle, x264_param_t *p_param )
{
    ...
    avis_input_t *h = (avis_input_t *)malloc( sizeof(avis_input_t) );
    if( !h )
        return -1;
...
int open_file_mkv( char *psz_filename, hnd_t *p_handle )
{
    ...
    *p_handle = NULL;
    ...
    p_mkv  = (mkv_t *)malloc( sizeof(*p_mkv) );
    if( !p_mkv )
        return -1;
...
int write_nalu_mkv( hnd_t handle, uint8_t *p_nalu, int i_size )
{
    ...
    switch( type )
    {
        ...
        case 0x07:
            if( !p_mkv->sps )
            {
                p_mkv->sps = (uint8_t *)malloc( i_size - 4 );
                if( !p_mkv->sps )
                    return -1;
        ...
        case 0x08:
            if( !p_mkv->pps )
            {
                p_mkv->pps = (uint8_t *)malloc( i_size - 4 );
                if( !p_mkv->pps )
                    return -1;
